...
// +kubebuilder:validation:XValidation:rule="!(has(self.type) && self.type == 'LocalDb' && (has(self.loginDatabase) || has(self.loginDatabaseRef) || has(self.loginDatabaseSelector)))",message="LocalDb users cannot specify loginDatabase, loginDatabaseRef, or loginDatabaseSelector"
type UserParameters struct {
	...
	// Type specifies the kind of user to create. Defaults to Local.
	// - Local: creates a server-level LOGIN and a database USER mapped to that login.
	// - AD: creates an Azure AD (external provider) database user.
	// - LocalDb: creates a contained database user directly with a password (no server-level login).
	// +optional
	// +kubebuilder:validation:Enum=Local;AD;LocalDb
